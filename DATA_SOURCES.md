...
- **対象**: 医科診療行為（M放射線治療、K843、D009/D413 等）都道府県別算定回数、および処方薬（注射）都道府県別薬効分類別数量の一部項目
...
> - 2026-08-04 再確認: `data/dashboard_data.json` の FY2024 D/K/M 搭載項目・全国総計 103項目は、`001712210.zip` 内の非公費ワークブックと全件一致（不一致0件）。`001712211.zip` は `(公費含む)` ワークブックで、多くの項目が `001712210.zip` より大きい値となるため、最新年度のアプリ内医科診療行為値は公費レセプトを含まないデータに基づくと確認した。

### 処方薬（注射）: ゾーフィゴ静注

2026-08-04 に `scripts/11_add_xofigo_drug_counts.py` で追加。DL済みの `NDB-Open/20260724` 台帳（`_スクリプト/inventory.jsonl`）から、注射薬の都道府県別薬効分類別数量ファイルを年度ごとに選択し、医薬品コード `622489201` / 薬価基準収載医薬品コード `4291432A1025` / 医薬品名 `ゾーフィゴ静注` を抽出した。

- 追加先: `data/dashboard_data.json` の `ndb.codes` に `RX 処方薬（注射・薬効分類別数量）`、`ndb.counts.pref` / `ndb.counts.national` に医薬品コード `622489201`
- 処理済み記録: `data/xofigo_drug_pref_counts.csv`, `data/xofigo_drug_sources.csv`
- 数量単位: `回分`
- 秘匿処理: 処方薬数量はファイル注記に基づき、400未満または逆算防止対象が `‐` 表示。抽出時は `‐` を欠損として保持し、同一都道府県で「0」と秘匿が混在して合算値が0になる場合は欠損扱いとした。
- 第10回（FY2023）: `処方薬_公費レセプト含まない（一括）_001499737.zip` を使用。対象ワークブックの注記に「公費レセプトは除く」とあり、ゾーフィゴ行は非掲載。
- 第11回（FY2024）: `処方薬_公費レセプト含まない（一括）_001711930.zip` を使用。対象ワークブックの注記に「公費レセプトは除く」とあり、ゾーフィゴ行は `注射薬 外来 (院内)` と `注射薬 入院` に掲載。
- FY2022 の `処方薬_注射_都道府県別薬効分類別数量_001258716.xlsx` でも、名称・医薬品コード・薬価基準収載コードの全セル検索で該当行は非掲載。

抽出された全国総計（総計列、秘匿前の全国値が掲載されている場合のみ）は次の通り。
