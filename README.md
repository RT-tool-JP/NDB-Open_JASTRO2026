# NDB Open Data 簡易集計サイト v2.3

サイト名は「NDB Open Data 簡易集計サイト」に統一している（2026-08-12）。
`index.html` の `<title>`・ヘッダー、ポータル（rt-info.org）のカードも同じ名称。
「Open Data」は英語表記のままにすること。
...
- **v2.3** (2026-08-11): スマートフォン対応。860px 以下でサイドバーをドロワー化、560px 以下で文字・余白を圧縮。地図上の統計パネルを開閉式にし（状態は localStorage に保持）、沖縄インセット枠の右辺を東経 129.6 度（`INSET_FRAME_RIGHT_LON`）に寄せた。表示値（分母）を全モードで常時編集可に変更。凡例の目盛り本数をバー幅から決定し重なりを解消、bbox 外に出る北海道（北方領土）の切れも解消。
